...
import {upload, InfoStatus} from "@/services/UploadService";
...
const DropZone = ({maxSize, availableTypes}: StandardComponentProps): JSX.Element => {
    ...
    const removeFile = (e: Event, item:IItem) => {
        ...
        if(item.status === InfoStatus.Loaded) {
            const newList:IItem[] | undefined = fileList?.filter((fileItem:IItem) => (fileItem.file.name !== item.file.name))
            setFileList(newList)
    ...
    const uploadFile = (fileItem: IItem) => new Promise((resolve, reject) => {
        if(fileItem.status !== InfoStatus.Loaded) {
            fileItem.status = InfoStatus.Started
            upload(fileItem.file, (e: AxiosProgressEvent):void => {
                ...
                setTimeout(() => {
                    setProgress(0)
                    fileItem.status = InfoStatus.Loaded
                    resolve(true)
                }, 0)
            })
        } else {
    ...
    const preventDrag = (e:DragEvent<HTMLFormElement>) => {
        e.preventDefault()
        setDragOver(true)
    }

    const preventLeaveDrag = (e:DragEvent<HTMLFormElement>) => {
        e.preventDefault()
        setDragOver(false)
    }
    ...
    <>
        <form action="/" onDrop={(e) => handleDrop(e)} onDragEnter={(e) => preventDrag(e)} onDragOver={(e) => preventDrag(e)} onDragLeave={(e) => preventLeaveDrag(e)}>
            <label htmlFor="fileInput" className={cn(styles.dropzone, {[styles.dropzone__dragover]: dragOver})}>
